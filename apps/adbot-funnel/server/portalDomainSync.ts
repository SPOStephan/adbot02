...
async function postPortal(token: string): Promise<Record<string, unknown> | null> {
  ...
  try {
    ...
    if (!response.ok || body.ok !== true) {
      console.warn("[portal-domain-sync] Funnel sync abgelehnt", {
    ...
    console.warn("[portal-domain-sync] Funnel sync unreachable", error);
    return null;
  }
}

/** Best-effort: never throws into Funnel request flow. */
export async function pushFunnelDomainUpsertToPortal(input: {
...
export async function pushFunnelDomainRevokeToPortal(input: {
  ownerUserId: string | null;
  hostname: string;
  toolDomainId: string;
}): Promise<void> {
...
export async function listPortalDomainsForFunnel(input: {
  ownerUserId: string | null;
