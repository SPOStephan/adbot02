...
- **SQL migrations:** When mentioning a Supabase/SQL migration, always paste the **complete** SQL migration source in the chat (raw code fence, no snippets/excerpts), and also include both the GitHub blob link and the raw link — do not wait to be asked. If chat size limits force a split, paste consecutive parts that concatenate to the full file and state the SHA256.
