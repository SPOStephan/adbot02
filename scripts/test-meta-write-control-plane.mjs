...
const lifetimeBudgetCanaryRegressionPath = join(
  scriptsDirectory,
  "test-meta-lifetime-budget-canary.sql",
);
...
try {
  ...
  const { stdout: lifetimeBudgetCanaryStdout } = await run(psqlPath, [
    ...psqlBase,
    "--file", lifetimeBudgetCanaryRegressionPath,
  ]);
  if (!lifetimeBudgetCanaryStdout.includes("Meta Lifetime budget canary checks passed")) {
    throw new Error("Lifetime budget canary regression did not emit its success marker");
  }
