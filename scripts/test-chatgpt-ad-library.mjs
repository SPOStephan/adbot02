...
assert.match(libraryTypes, /img\.chatgptadlibrary\.com/);
...
assert.match(importService, /uniquifyJpegBytes/);
...
assert.match(retrieval, /count: "exact"/);
assert.match(retrieval, /max-rows|100_000/);
assert.match(retrieval, /loadChatGPTAdLibraryPage/);
assert.match(retrieval, /\.range\(/);
assert.doesNotMatch(retrieval, /\.limit\(2000\)/);
assert.doesNotMatch(retrieval, /\.limit\(500\)/);
...
assert.match(intelligenceApi, /loadChatGPTAdLibraryPage/);
assert.match(intelligenceApi, /offset/);
assert.match(page, /ChatGPTAdLibraryImportPanel/);
assert.match(page, /initialHits/);
assert.match(page, /initialHitsTotal/);
assert.match(page, /getChatGPTAdLibraryCrawlStatus/);
assert.match(page, /loadChatGPTAdLibraryPage/);
assert.match(panel, /InspirationPager/);
assert.match(panel, /CHATGPT_AD_LIBRARY_PAGE_SIZE/);
assert.match(panel, /Wiederkehrender Scrape/);
assert.match(panel, /15 Minuten/);
assert.match(panel, /GitHub Action alle 2 Stunden/);
assert.doesNotMatch(panel, /Cron jede Minute/);
assert.doesNotMatch(panel, /800-Sekunden/);
