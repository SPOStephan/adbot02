...
assert.match(constants, /CHATGPT_AD_LIBRARY_UNLOCK_BATCH_MAX = 20/);
assert.match(constants, /CHATGPT_AD_LIBRARY_UNLOCK_CONCURRENCY = 5/);
assert.match(constants, /CHATGPT_AD_LIBRARY_UNLOCK_ROUNDS_MAX = 2/);
...
assert.match(vercel, /chatgpt-ad-library-scrape",\s*"schedule": "\*\/5 \* \* \* \*"/);
...
assert.match(adminCrawl, /maxDuration = 180/);
...
assert.match(docs, /20 Ads|bis 20 Ads/);
assert.match(docs, /5 min/);
assert.match(panel, /parallel/);
