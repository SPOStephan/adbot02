...
assert.match(constants, /CHATGPT_AD_LIBRARY_DISCOVER_COOLDOWN_MS/);
...
assert.match(read("src/app/dashboard/inspiration/page.tsx"), /withTimeout/);
assert.match(panel, /Queue leer/);
...
assert.match(adminCrawl, /scrapeChatGPTAdLibraryUnlockDrain/);
assert.match(adminCrawl, /CHATGPT_AD_LIBRARY_RUN_NOW_ROUNDS/);
assert.match(adminCrawl, /CHATGPT_AD_LIBRARY_RUN_NOW_BUDGET_MS/);
assert.match(adminCrawl, /requireLease: false/);
assert.match(scrape, /last_failure_errors/);
assert.match(panel, /Sofortlauf gestartet/);
assert.match(panel, /Im Vault jetzt/);
assert.match(panel, /neu im Vault/);
assert.match(panel, /CHATGPT_AD_LIBRARY_RUN_NOW_ROUNDS/);
...
assert.match(constants, /CHATGPT_AD_LIBRARY_RUN_NOW_ROUNDS = 6/);
assert.match(constants, /CHATGPT_AD_LIBRARY_RUN_NOW_BUDGET_MS = 240_000/);
