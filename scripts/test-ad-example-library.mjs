...
const pager = read("src/components/InspirationPager.tsx");
...
assert.match(types, /AD_EXAMPLE_PAGE_SIZE/);
assert.match(types, /structureKind/);
assert.match(client, /structureKind/);
assert.match(client, /Tag filtern/);
assert.match(service, /loadAdExamplesPage/);
assert.match(service, /\.range\(/);
assert.doesNotMatch(service, /\.limit\(500\)/);
assert.match(api, /loadAdExamplesPage/);
assert.match(page, /loadAdExamplesPage/);
assert.match(client, /InspirationPager/);
assert.match(client, /\/api\/admin\/ad-examples\?/);
assert.match(pager, /Anfang/);
assert.match(pager, /Zurück/);
assert.match(pager, /Weiter/);
assert.match(pager, /Ende/);
