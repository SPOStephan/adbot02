...
const BUCKET_OPTIONS = {
  ...
  allowedMimeTypes: [...SITE_LOGO_RECOMMENDATIONS.allowedMimeTypes],
};
...
export async function removeSiteLogoObject(path: string | null | undefined): Promise<void> {
  if (!path) return;
  const admin = createAdminClient();
...
export function publicUrlForSiteLogoPath(path: string | null | undefined): string | null {
  if (!path) return null;
