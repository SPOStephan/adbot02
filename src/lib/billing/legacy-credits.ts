...
import { type CreditActionKey, type CreditBalance } from "./credit-contract";
import { InsufficientCreditsError } from "./credit-errors";

export type LegacyCreditReservation = {
...
function parseReservationRow(data: unknown): LegacyCreditReservation {
...
export async function reserveCredits(input: {
  ...
  referenceId?: string;
  ttlSeconds?: number;
}): Promise<LegacyCreditReservation> {
  const admin = createAdminClient();
...
export async function withCreditReservation<T>(input: {
  ...
  referenceType?: string;
  referenceId?: string;
  run: (reservation: LegacyCreditReservation) => Promise<T>;
}): Promise<T> {
