...
import type { CampaignBriefView } from "@/components/CampaignAssistantBrief";
import type {
  AllowedDomainView,
...
import type {
  ContentBoostOverrideView,
  HeldOrganicBoostPlanView,
...
import type { MetaConnectedAssetView } from "@/components/MetaConnectedAssets";
...
export type CustomerDashboardSearchParams = {
  meta?: string | string[];
...
async function loadCustomerDashboardImpl(
  ...
  const metaNotice = getMetaNotice(
    firstQueryValue(searchParams.meta),
    firstQueryValue(searchParams.meta_error),
    metaConnected,
    writeScopeGranted,
    firstQueryValue(searchParams.meta_missing_scopes),
    firstQueryValue(searchParams.meta_unexpected_scopes),
    firstQueryValue(searchParams.meta_callback_stage),
  );
  const [
    { data: liveCampaigns },
...
export type CustomerDashboardData = Awaited<
  ReturnType<typeof loadCustomerDashboardImpl>
>;
