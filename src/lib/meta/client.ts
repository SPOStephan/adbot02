...
export type MetaInstagramAccountAsset = {
  id: string;
  name: string;
  username: string | null;
};
...
function parseInstagramAccountAsset(
  value: unknown,
): MetaInstagramAccountAsset | null {
  if (!isRecord(value)) {
    return null;
  }
...
export async function getMetaPageAssets(input: {
  ...
  // to the token". Only omit allowedPageIds for intentional unfiltered listing.
  return {
...
export async function getMetaInstagramAccountAssets(input: {
  accessToken: string;
  appSecret: string;
  allowedInstagramAccountIds: Set<string>;
  ...
  for (const id of ids) {
    const url = new URL(
      `/${META_GRAPH_VERSION}/${encodeURIComponent(id)}`,
      META_GRAPH_ORIGIN,
    );
...
export async function getMetaConnectionAssets(input: {
  ...
  // Empty allow-list means no selected ad accounts — never fall open.
