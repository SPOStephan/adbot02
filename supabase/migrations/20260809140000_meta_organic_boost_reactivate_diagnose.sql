-- Force-reactivate found 0 candidates even when Meta shows PAUSED Beitrag-Push
-- campaigns. Root causes we harden here:
-- 1) Candidate discovery required an existing MANAGED automation_targets row
--    (SUSPENDED / missing → invisible).
-- 2) queue_internal resolved campaigns via stale automation_targets.campaign_id
--    instead of current platform_campaign_id.
-- 3) BLOCKED outcomes (snapshot/policy) looked like "no paused campaigns".
-- There is NO 24h-remaining filter — only stop_time already passed.
...
begin
  ...
  -- Prefer current row by Meta campaign id (targets.campaign_id can be stale).
...
declare
  ...
  v_linked integer := 0;
  v_active_local integer := 0;
  v_adset_paused_only integer := 0;
  v_targets_repaired integer := 0;
  v_remaining_under_24h integer := 0;
  v_missing_current integer := 0;
begin
  ...
  -- Same source of truth as the dashboard list: organic-boost links + bindings.
  for v_candidate in
    select distinct on (coalesce(binding.remote_object_id, c.platform_campaign_id))
      c.id as campaign_id,
      coalesce(binding.remote_object_id, c.platform_campaign_id)
    ...
    join public.mutation_plans boost_plan
      on boost_plan.id = link.plan_id
     and boost_plan.user_id = link.user_id
     and boost_plan.platform_account_id = link.platform_account_id
     and boost_plan.source_rule_key = 'organic-boost'
     and boost_plan.action_type = 'LAUNCH_CHAIN'
    ...
    left join public.campaigns c
     ...
     and (
       (
         ...
         and c.platform_campaign_id = binding.remote_object_id
       )
       or (
         binding.local_campaign_id is not null
         and c.id = binding.local_campaign_id
       )
     )
    left join public.automation_targets target
      on target.platform_account_id = p_platform_account_id
     and target.user_id = p_user_id
     and target.target_type = 'CAMPAIGN'
     and target.platform_object_id = coalesce(
       binding.remote_object_id,
  ...
  loop
    ...
    if v_candidate.platform_campaign_id is null
      ...
      v_missing_current := v_missing_current + 1;
      continue;
    end if;

    if v_candidate.stop_time is not null
      and v_candidate.stop_time > p_planned_at
      and v_candidate.stop_time <= p_planned_at + interval '24 hours' then
      v_remaining_under_24h := v_remaining_under_24h + 1;
    end if;

    if upper(coalesce(v_candidate.effective_status, '')) = 'ADSET_PAUSED'
      and upper(coalesce(v_candidate.campaign_status, '')) = 'ACTIVE' then
      v_adset_paused_only := v_adset_paused_only + 1;
    end if;

    if upper(coalesce(v_candidate.campaign_status, '')) <> 'PAUSED'
      and upper(coalesce(v_candidate.effective_status, ''))
        not in ('PAUSED', 'CAMPAIGN_PAUSED') then
      v_active_local := v_active_local + 1;
      continue;
    end if;
    ...
    v_scope_key := 'campaign:' || v_candidate.platform_campaign_id;
    ...
    insert into public.automation_targets (
      ...
      p_user_id, p_platform_account_id, 'CAMPAIGN', v_scope_key,
      v_candidate.platform_campaign_id, v_scope_key, 'CAMPAIGN', v_scope_key,
      v_candidate.campaign_id, null, null, 'MANAGED',
      p_planned_at, p_planned_at, p_planned_at
    )
    ...
    do update set
      ...
      campaign_scope_key = coalesce(
        public.automation_targets.campaign_scope_key,
        excluded.campaign_scope_key
      ),
      row_version = public.automation_targets.row_version + 1,
      updated_at = p_planned_at
    returning id into v_target_id;

    if v_candidate.automation_target_id is null
      or coalesce(v_candidate.target_status, '') <> 'MANAGED'
      or v_candidate.automation_target_id is distinct from v_target_id then
      v_targets_repaired := v_targets_repaired + 1;
    end if;
    ...
    if v_target_id is null then
      ...
      where target.platform_account_id = p_platform_account_id
        and target.target_type = 'CAMPAIGN'
        and target.platform_object_id = v_candidate.platform_campaign_id;
    end if;
    ...
    v_result := public.queue_meta_organic_boost_reactivate_internal(
      ...
      jsonb_build_object(
        ...
        'resume_reason', 'organic_boost_reactivate',
        'campaign_scope_key', v_scope_key,
  ...
  perform public.append_meta_mutation_audit_event(
    ...
    jsonb_build_object(
      ...
      'linked', v_linked,
      'active_local', v_active_local,
      'adset_paused_only', v_adset_paused_only,
      'targets_repaired', v_targets_repaired,
      'remaining_under_24h', v_remaining_under_24h,
      'missing_current', v_missing_current
    ),
    '{}'::jsonb,
    null, null, null, null, null, p_planned_at
  );
...
comment on function public.force_reactivate_paused_meta_organic_boost_campaigns(
  ...
  'Queues ACTIVATE for PAUSED Beitrag-Push campaigns from meta_organic_boost_links; repairs MANAGED targets; no 24h-remaining gate.';
