-- Creative Generation Phase 5: style reference_asset_ids enqueue gate.
-- Allows marked_good / performance_winner / STYLE_REFERENCE (customer) and
-- Inspiration Vault STYLE_REFERENCE rows. Max 4 refs.
...
begin
  ...
  if coalesce(p_input_payload->>'contract_version', '') = 'adbot-creative-generation-v1' then
    ...
    -- Phase 5: style references ownership / vault gate.
...
revoke all on function public.creative_generation_style_references_allowed(
  uuid, uuid, jsonb
...
grant execute on function public.creative_generation_style_references_allowed(
  uuid, uuid, jsonb
) to service_role;
...
comment on function public.creative_generation_style_references_allowed(
  uuid, uuid, jsonb
) is
  'Phase 5: true iff reference_asset_ids empty or all are allowed style corpus rows (max 4).';

comment on function public.enqueue_creative_asset_job(
  uuid, uuid, uuid, text, text, text, jsonb, integer
) is
  'Enqueue creative asset job. Phase 5: validates style reference_asset_ids when present.';
