-- Organic boost planner: do not hard-fail when the exact marketing sync
-- has no COMPLETE exposure snapshot yet. Prefer the matching snapshot, then
-- fall back to the latest COMPLETE EUR snapshot for the active policy.
-- Pass that snapshot's marketing sync id into materialize so reservation gates match.
...
begin
  ...
  if not exists (
    ...
    return jsonb_build_object('status', 'LEASE_REQUIRED');
  end if;
  ...
  if not found then
    -- Fresh marketing sync may not have a budget snapshot yet; reuse the
    -- newest complete exposure snapshot for this policy so Beitrag-Push can
    -- start immediately after posts are recognized.
    select snapshot.* into v_snapshot
    from public.daily_budget_exposure_snapshots snapshot
    where snapshot.user_id = p_user_id
      and snapshot.platform_account_id = p_platform_account_id
  ...
  if not found then
    -- Policy may have been rotated after the last budget snapshot; any recent
    -- COMPLETE EUR snapshot for the account is enough to gate remaining budget.
    select snapshot.* into v_snapshot
    from public.daily_budget_exposure_snapshots snapshot
    where snapshot.user_id = p_user_id
      and snapshot.platform_account_id = p_platform_account_id
...
comment on function public.run_meta_organic_boost_planner(uuid, uuid, uuid, uuid, timestamptz) is
  'After content/marketing sync: materialize organic boost plans for new candidates. Uses the matching exposure snapshot when present, otherwise the latest COMPLETE account snapshot.';
