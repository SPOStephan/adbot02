...
-- Structural multi-ad Step 2b: also support 2 Ad Sets
-- (1 campaign → 2 ad sets → 1 ad each), ABO budget split half/half.
-- Keeps Step 2a path (2 ads in 1 ad set) and default single-ad path.
-- Mutually exclusive with DCA. Does NOT redefine organic-boost materialize.
--
-- Approve step graphs: 20/21/28/29/33/34. Reconcile requires 2 AD_SET bindings
-- when structural_ad_set_count=2. Copied forward from 20260818190000.
...
declare
  ...
  v_structural_ad_set_count integer := 1;
  ...
  v_ad_set_payload_2 jsonb;
  v_creative_name_2 text;
  v_ad_name_2 text;
  v_ad_set_name_2 text;
  v_budget_half_1 bigint;
  v_budget_half_2 bigint;
  ...
  v_step_validate_ad_set_2 uuid := gen_random_uuid();
  v_step_create_ad_set_2 uuid := gen_random_uuid();
  v_step_read_ad_set_paused_2 uuid := gen_random_uuid();
  ...
  v_step_activate_ad_set_2 uuid := gen_random_uuid();
  v_step_read_ad_set_active_2 uuid := gen_random_uuid();
...
begin
  ...
  v_structural_ad_set_count := 1;
  if coalesce(p_launch_inputs, '{}'::jsonb) ? 'structural_ad_set_count' then
    begin
      v_structural_ad_set_count := (p_launch_inputs->>'structural_ad_set_count')::integer;
    exception when others then
      raise exception 'structural_ad_set_count muss 1 oder 2 sein';
    end;
  end if;
  if v_structural_ad_set_count not in (1, 2) then
    raise exception 'structural_ad_set_count muss 1 oder 2 sein';
  end if;
  if v_structural_ad_set_count = 2 and v_structural_ad_count <> 2 then
    raise exception '2 Ad Sets erfordern structural_ad_count=2';
  end if;
  ...
  if v_structural_ad_set_count = 2 then
    if p_budget_owner_type <> 'AD_SET' then
      raise exception '2 Ad Sets erfordern Budgetträger AD_SET (kein CBO)';
    end if;
    if p_daily_budget_minor < 200 then
      raise exception 'Für 2 Ad Sets muss das Tagesbudget mindestens 2,00 EUR betragen (hälftige Verteilung, je mind. 1,00 EUR)';
    end if;
    v_budget_half_1 := p_daily_budget_minor / 2;
    v_budget_half_2 := p_daily_budget_minor - v_budget_half_1;
    if v_budget_half_1 < 100 or v_budget_half_2 < 100 then
      raise exception 'Jede Anzeigengruppe braucht mindestens 1,00 EUR Tagesbudget nach der Teilung';
    end if;
    v_ad_set_payload := jsonb_set(
      v_ad_set_payload,
      '{daily_budget}',
      pg_catalog.to_jsonb(v_budget_half_1::text),
      true
    );
  end if;
  ...
  v_dco_image_refs := jsonb_build_array(jsonb_build_object('hash', v_image_reference));
  ...
  -- Dynamic Creative text variants and/or optional multi-image asset_feed_spec.
  ...
  if v_structural_ad_set_count = 2 then
    -- Clone enriched ad set 1 (half1 budget already applied) then override half2 + name.
    v_ad_set_payload_2 := v_ad_set_payload;
    v_ad_set_payload_2 := jsonb_set(
      v_ad_set_payload_2,
      '{daily_budget}',
      pg_catalog.to_jsonb(v_budget_half_2::text),
      true
    );
    v_ad_set_name_2 := left(v_ad_set_payload_2->>'name', 240);
    if v_ad_set_name_2 ~ '\[[0-9a-f]{12}-s\]$' then
      v_ad_set_name_2 := regexp_replace(v_ad_set_name_2, ' \[[0-9a-f]{12}-s\]$', '');
    end if;
    v_ad_set_name_2 := left(v_ad_set_name_2, 240)
      || ' [' || v_tracking_suffix || '-s2]';
    v_ad_set_payload_2 := jsonb_set(
      v_ad_set_payload_2, '{name}', pg_catalog.to_jsonb(v_ad_set_name_2), true
    );
  end if;
  ...
  if v_structural_ad_count = 2 then
    if v_structural_ad_set_count = 2 then
      v_planned_payload := v_planned_payload || jsonb_build_object(
        ...
        'ad_sets', jsonb_build_array(v_ad_set_payload, v_ad_set_payload_2),
  ...
  if v_structural_ad_set_count = 2 then
    v_request := jsonb_build_object(
      'operation', 'CREATE_AD_SET', 'object_type', 'AD_SET',
      'mode', 'validate_only', 'payload', v_ad_set_payload_2
    );
    insert into public.mutation_plan_steps (
      id, plan_id, user_id, platform_account_id, step_index, step_key,
      operation, object_type, depends_on_step_id, planned_request,
      ...
      v_step_validate_ad_set_2, v_plan_id, p_user_id, p_platform_account_id,
      v_index, 'validate-ad-set-2', 'VALIDATE', 'AD_SET', v_previous_step,
      v_request, public.meta_sha256(v_request::text),
      jsonb_build_object('validated', true), 'NONE', 'PENDING'
    );
    v_previous_step := v_step_validate_ad_set_2;
    v_index := v_index + 1;

    v_request := jsonb_build_object(
      'operation', 'CREATE_AD_SET', 'object_type', 'AD_SET',
      'mode', 'execute', 'payload', v_ad_set_payload_2
    );
    insert into public.mutation_plan_steps (
      id, plan_id, user_id, platform_account_id, step_index, step_key,
      operation, object_type, depends_on_step_id, planned_request,
      ...
      v_step_create_ad_set_2, v_plan_id, p_user_id, p_platform_account_id,
      v_index, 'create-ad-set-paused-2', 'CREATE', 'AD_SET', v_previous_step,
      v_request, public.meta_sha256(v_request::text),
      jsonb_build_object('status', 'PAUSED'), 'PAUSE', 'PENDING'
    );
    v_previous_step := v_step_create_ad_set_2;
    v_index := v_index + 1;

    v_request := jsonb_build_object(
      'operation', 'READ', 'object_type', 'AD_SET',
      'object_id', jsonb_build_object('$binding_step_id', v_step_create_ad_set_2)
    );
    insert into public.mutation_plan_steps (
      id, plan_id, user_id, platform_account_id, step_index, step_key,
      operation, object_type, depends_on_step_id, planned_request,
      ...
      v_step_read_ad_set_paused_2, v_plan_id, p_user_id, p_platform_account_id,
      v_index, 'read-ad-set-paused-2', 'READ', 'AD_SET', v_previous_step,
      v_request, public.meta_sha256(v_request::text),
      jsonb_build_object('status', 'PAUSED'), 'NONE', 'PENDING'
    );
    v_previous_step := v_step_read_ad_set_paused_2;
    v_index := v_index + 1;
  end if;
  ...
  if v_structural_ad_set_count = 2 then
    ...
    insert into public.mutation_plan_steps (
      ...
      v_step_activate_ad_set_2, v_plan_id, p_user_id, p_platform_account_id,
      v_index, 'activate-ad-set-2', 'UPDATE', 'AD_SET', v_previous_step,
      v_request, public.meta_sha256(v_request::text),
      jsonb_build_object('status', 'ACTIVE'), 'PAUSE', 'PENDING'
    );
    v_previous_step := v_step_activate_ad_set_2;
    v_index := v_index + 1;
    ...
    insert into public.mutation_plan_steps (
      ...
      v_step_read_ad_set_active_2, v_plan_id, p_user_id, p_platform_account_id,
      v_index, 'read-ad-set-active-2', 'READ', 'AD_SET', v_previous_step,
      v_request, public.meta_sha256(v_request::text),
      jsonb_build_object('status', 'ACTIVE'), 'NONE', 'PENDING'
    );
    v_previous_step := v_step_read_ad_set_active_2;
    v_index := v_index + 1;
  end if;
...
begin
  ...
  if v_structural_ad_set_count = 2 then
    raise exception '2 Ad Sets sind nur mit Tagesbudget und AD_SET-Budgetträger erlaubt';
  end if;
...
begin
  ...
  if jsonb_typeof(v_plan.planned_payload->'brand_asset_ids') <> 'array'
  ...
  if v_step_count <> v_base_step_count + v_upload_step_count
    ...
    or exists (
      ...
      where step.plan_id = v_plan.id
        and step.step_key like 'create-ad-set-paused%'
    ...
    or exists (
      select 1 from public.mutation_plan_steps step
      where step.plan_id = v_plan.id
        and step.step_key like 'activate-ad-set%'
        and step.planned_request->>'status' is distinct from 'ACTIVE'
    ) then
...
declare
  ...
  v_ad_set_binding_count integer := 0;
  v_ad_set_remote_ids text[] := '{}'::text[];
  v_loop_ad_group_id uuid;
  v_loop_ad_set_budget bigint;
...
begin
  ...
  v_structural_ad_set_count := coalesce(
    (v_plan.planned_payload->>'structural_ad_set_count')::integer,
    1
  );
  if v_structural_ad_set_count not in (1, 2) then
    raise exception 'Unsupported structural_ad_set_count for reconciliation';
  end if;
  if v_structural_ad_set_count = 2 and v_structural_ad_count <> 2 then
    raise exception 'Invalid structural topology for reconciliation';
  end if;
  ...
  select count(*)::integer,
         coalesce(array_agg(binding.remote_object_id order by binding.bound_at, binding.id), '{}'::text[])
    into v_ad_set_binding_count, v_ad_set_remote_ids
  from public.remote_object_bindings binding
  where binding.plan_id = v_plan.id
    and binding.user_id = v_plan.user_id
    and binding.platform_account_id = v_plan.platform_account_id
  ...
  if v_campaign_binding.id is null
    ...
    or v_ad_set_binding_count <> v_structural_ad_set_count
  ...
  v_matches := v_campaign_snapshot.id is not null
    ...
    and (
      ...
      or (
        ...
        and v_structural_ad_set_count = 1
      and (
        select count(*)::integer
      ...
      or (
        v_structural_ad_count = 2
        and v_structural_ad_set_count = 2
        and (
          select count(*)::integer
          from public.remote_object_bindings as_binding
          join lateral (
            select s.*
            from public.meta_mutation_remote_snapshots s
            join public.mutation_plan_steps snapshot_step on snapshot_step.id = s.step_id
            where s.plan_id = v_plan.id
              and s.object_type = 'AD_SET'
              and s.remote_object_id = as_binding.remote_object_id
              and s.snapshot_kind in ('READ_AFTER_WRITE', 'AMBIGUITY_PROBE')
            order by snapshot_step.step_index desc, s.observed_at desc, s.created_at desc
            limit 1
          ) as_snap on true
          where as_binding.plan_id = v_plan.id
            and as_binding.object_type = 'AD_SET'
            and as_binding.remote_object_id = any (v_ad_set_remote_ids)
            and coalesce(
              as_snap.snapshot_payload->>'status',
              as_snap.snapshot_payload->>'effective_status'
            ) = 'ACTIVE'
            and as_snap.snapshot_payload->>'campaign_id'
              = v_campaign_binding.remote_object_id
        ) = 2
        and (
          select count(*)::integer
          ...
          where ad_binding.plan_id = v_plan.id
            ...
            and ad_snap.snapshot_payload->>'campaign_id'
              = v_campaign_binding.remote_object_id
            and ad_snap.snapshot_payload->>'adset_id' = any (v_ad_set_remote_ids)
            and coalesce(
              ad_snap.snapshot_payload#>>'{creative,id}',
              ad_snap.snapshot_payload->>'creative_id'
            ) = any (v_creative_remote_ids)
        ...
        and (
          select count(distinct ad_snap.snapshot_payload->>'adset_id')::integer
          from public.remote_object_bindings ad_binding
          join lateral (
            select s.*
            from public.meta_mutation_remote_snapshots s
      ...
      (
        ...
        and (
          ...
          (v_budget_owner_type = 'AD_SET'
            ...
            and (
              select coalesce(sum(
                nullif((as_snap.snapshot_payload->>'daily_budget')::bigint, 0)
              ), 0)::bigint
              from public.remote_object_bindings as_binding
              join lateral (
                select s.*
              ...
              where as_binding.plan_id = v_plan.id
                and as_binding.object_type = 'AD_SET'
                and as_binding.remote_object_id = any (v_ad_set_remote_ids)
                and coalesce(as_snap.snapshot_payload->>'daily_budget', '') ~ '^[0-9]+$'
            ) = v_expected_budget)
        )
      )
      or
  ...
  if v_structural_ad_set_count = 2 then
    for v_loop_binding in
      ...
      where binding.plan_id = v_plan.id
        ...
        and binding.platform_account_id = v_plan.platform_account_id
        and binding.object_type = 'AD_SET'
        and binding.remote_object_id = any (v_ad_set_remote_ids)
        and binding.id <> v_ad_set_binding.id
      order by binding.bound_at, binding.id
    loop
      select s.* into v_loop_snapshot
      from public.meta_mutation_remote_snapshots s
      ...
      v_loop_ad_set_budget := null;
      if coalesce(v_loop_snapshot.snapshot_payload->>'daily_budget', '') ~ '^[0-9]+$' then
        v_loop_ad_set_budget := nullif(
          (v_loop_snapshot.snapshot_payload->>'daily_budget')::bigint,
          0
        );
      end if;
      ...
      insert into public.ad_groups (
        ...
        coalesce(
          nullif(v_loop_snapshot.snapshot_payload->>'name', ''),
          v_plan.planned_payload#>>'{ad_sets,1,name}',
          v_plan.planned_payload#>>'{ad_set,name}'
        ),
        'ACTIVE',
        coalesce(v_loop_snapshot.snapshot_payload->>'effective_status', 'ACTIVE'),
        v_loop_snapshot.snapshot_payload->>'optimization_goal',
        v_loop_snapshot.snapshot_payload->>'billing_event',
        v_loop_snapshot.snapshot_payload->>'destination_type',
        v_loop_ad_set_budget,
        null,
        case
          when coalesce(v_loop_snapshot.snapshot_payload->>'bid_amount', '') ~ '^[0-9]+$'
            then (v_loop_snapshot.snapshot_payload->>'bid_amount')::bigint
          else null
        end,
        v_loop_snapshot.snapshot_payload->>'bid_strategy',
        null,
        null,
        case
          when coalesce(v_loop_snapshot.snapshot_payload->>'updated_time', '')
      ...
      do update set
        ...
        is_current = true,
        updated_at = now()
      returning id into v_loop_ad_group_id;

      update public.remote_object_bindings
      set local_campaign_id = v_campaign_id,
          local_ad_group_id = v_loop_ad_group_id,
          reconciled_at = now()
      where id = v_loop_binding.id;
      ...
      insert into public.automation_targets (
        ...
        v_plan.user_id, v_plan.platform_account_id, 'AD_SET',
        'adset:' || v_loop_binding.remote_object_id,
        v_loop_binding.remote_object_id, v_canonical_scope_key,
        case when v_budget_owner_type = 'AD_SET' then 'AD_SET' else null end,
        case when v_budget_owner_type = 'AD_SET'
          then 'adset:' || v_loop_binding.remote_object_id else null end,
        v_campaign_id, v_loop_ad_group_id, null, 'MANAGED', now(), now(), now()
      ) on conflict (platform_account_id, target_type, platform_object_id)
      do update set
        target_key = excluded.target_key,
        campaign_scope_key = excluded.campaign_scope_key,
    ...
    -- Re-bind second ad (+ its automation target) onto its own ad group.
    for v_loop_binding in
      select binding.*
      from public.remote_object_bindings binding
      where binding.plan_id = v_plan.id
    ...
    loop
      ...
      select ag.id into v_loop_ad_group_id
      from public.ad_groups ag
      where ag.platform_account_id = v_plan.platform_account_id
        and ag.platform_ad_group_id = v_loop_snapshot.snapshot_payload->>'adset_id'
        and ag.is_current
      limit 1;

      if v_loop_ad_group_id is not null then
        update public.ads
        set ad_group_id = v_loop_ad_group_id,
            updated_at = now()
        where platform_account_id = v_plan.platform_account_id
          and platform_ad_id = v_loop_binding.remote_object_id;
        ...
        where platform_account_id = v_plan.platform_account_id
          and target_type = 'AD'
          and platform_object_id = v_loop_binding.remote_object_id;

        update public.remote_object_bindings
        set local_ad_group_id = v_loop_ad_group_id,
            reconciled_at = now()
        where id = v_loop_binding.id;
      end if;
    end loop;
