-- Harden customer launch prepare against opaque "EUR Meta snapshot" failures:
-- 1) Coerce unknown/null Meta timezone names to Europe/Berlin (persist).
-- 2) Raise specific gate errors instead of one catch-all.
-- 3) Bootstrap exposure snapshot inside materialize when missing.

begin;

create or replace function public.normalize_meta_account_timezone_name(
...
revoke all on function public.normalize_meta_account_timezone_name(text)
  from public, anon, authenticated;
grant execute on function public.normalize_meta_account_timezone_name(text)
  to service_role;

comment on function public.normalize_meta_account_timezone_name(text) is
  'Returns a pg_timezone_names-safe timezone; falls back to Europe/Berlin.';

create or replace function public.trg_normalize_platform_account_timezone()
returns trigger
language plpgsql
security definer
set search_path = ''
as $$
begin
  -- Always persist a pg_timezone_names-safe value. Meta occasionally returns
  -- names that break launch materialize (`AT TIME ZONE` / snapshot day).
  new.marketing_timezone_name := public.normalize_meta_account_timezone_name(
    new.marketing_timezone_name
  );
  return new;
end;
$$;

drop trigger if exists trg_normalize_platform_account_timezone
  on public.platform_accounts;
create trigger trg_normalize_platform_account_timezone
  before insert or update of marketing_timezone_name
  on public.platform_accounts
  for each row
  execute function public.trg_normalize_platform_account_timezone();

-- Repair any currently stored unknown timezone names before the next prepare.
update public.platform_accounts account
set marketing_timezone_name = public.normalize_meta_account_timezone_name(
  account.marketing_timezone_name
)
where account.platform = 'meta'
  and account.revoked_at is null
  and (
    account.marketing_timezone_name is null
    or account.marketing_timezone_name = ''
    or not exists (
      select 1
      from pg_catalog.pg_timezone_names tz
      where tz.name = account.marketing_timezone_name
    )
  );
...
comment on function public.materialize_meta_customer_launch_plan(
  ...
  'Customer daily launch prepare with timezone coerce and exposure bootstrap.';
