-- Name the ACCOUNT_UNAVAILABLE readiness failures (currency / scopes /
-- missing sync_id / stale last_success) so reconnect + Abruf issues are
-- actionable instead of a generic EUR/ads_management/48h blob.
...
begin
  ...
  if not found then
    ...
    v_last_error := concat_ws(
      '; ',
      case
        when v_account.id is null or v_account.revoked_at is not null
          then 'Konto fehlt oder widerrufen'
        else null
      end,
      case
        when coalesce(v_account.marketing_currency, '') is distinct from 'EUR'
          then format(
            'Währung=%s (EUR nötig — oft nach Asset-Reconnect ohne erfolgreichen Kampagnenabruf)',
            coalesce(nullif(v_account.marketing_currency, ''), 'null')
          )
        else null
      end,
      case
        when not ('ads_management' = any(coalesce(v_account.meta_scopes, array[]::text[])))
          then 'Scope ads_management fehlt'
        else null
      end,
      case
        when v_account.marketing_sync_id is null
          then 'marketing_sync_id fehlt (Kampagnenabruf noch nicht erfolgreich)'
        else null
      end,
      case
        when v_account.marketing_last_success_at is null
          then 'marketing_last_success_at fehlt'
        when v_account.marketing_last_success_at < now() - interval '48 hours'
          then format(
            'Marketing-Sync veraltet (%s)',
            v_account.marketing_last_success_at
          )
        when v_account.marketing_last_success_at > now() + interval '1 minute'
          then 'marketing_last_success_at liegt in der Zukunft'
        else null
      end
    );

    if v_last_error is null or length(trim(v_last_error)) = 0 then
      v_last_error :=
        'Meta-Konto nicht bereit (EUR, ads_management oder Marketing-Sync >48h).';
    end if;

    v_detail := jsonb_build_object(
      'status', v_status,
      'last_error', to_jsonb(v_last_error::text)
    );
    update public.platform_accounts
    set
      organic_boost_planner_status = v_status,
  ...
  if not v_has_snapshot then
  ...
  elsif v_snapshot.policy_id is distinct from v_policy.id then
    begin
      v_snapshot := public.ensure_meta_organic_boost_exposure_snapshot(
        p_platform_account_id,
        p_user_id,
        v_policy.id,
        coalesce(v_snapshot.source_marketing_sync_id, p_source_marketing_sync_id),
        v_lease_token,
        p_planned_at
      );
      if v_snapshot.id is not null then
    ...
    exception when others then
      v_last_error := SQLERRM;
      -- Keep previous COMPLETE snapshot; materialize tolerates policy rotation.
    end;
  end if;

  if not v_has_snapshot or v_snapshot.id is null then
    ...
    v_detail := jsonb_build_object(
      ...
      'last_error', to_jsonb(
        coalesce(
          v_last_error,
          'Kein COMPLETE Budget-Exposure-Snapshot für die aktuelle Policy.'
        )::text
      )
    );
    update public.platform_accounts
  ...
  v_snapshot_sync_id := coalesce(
    v_snapshot.source_marketing_sync_id,
    p_source_marketing_sync_id
  );
  ...
  loop
    ...
    begin
      ...
      if coalesce(v_result->>'outcome', '') = 'QUEUED' then
        v_created := v_created + 1;
      elsif coalesce(v_result->>'outcome', '') = 'EXISTING' then
  ...
  v_detail := jsonb_build_object(
    ...
    'last_error', case
      when v_last_error is null then 'null'::jsonb
      else to_jsonb(v_last_error)
    end
  );
...
exception when others then
  ...
  v_detail := jsonb_build_object(
    ...
    'candidates_failed', 0,
    'candidates_considered', 0,
    'last_error', to_jsonb(coalesce(SQLERRM, 'unknown planner error')::text),
    'sqlstate', to_jsonb(coalesce(SQLSTATE, '')::text)
  );
  begin
    update public.platform_accounts
    set
      organic_boost_planner_status = v_status,
      organic_boost_planner_detail = v_detail,
      organic_boost_planner_last_run_at = coalesce(p_planned_at, now()),
      updated_at = now()
    where id = p_platform_account_id and user_id = p_user_id;
  exception when others then
    null;
...
comment on function public.run_meta_organic_boost_planner(uuid, uuid, uuid, uuid, timestamptz) is
  'Materialize organic boost plans; typed JSON; ACCOUNT_UNAVAILABLE lists concrete readiness gaps.';
