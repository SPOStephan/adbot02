-- Beitrag-Push planner must return structured status JSON instead of raising
-- to the API layer (which only showed PLANNER_RPC_FAILED). Soft-fail snapshot
-- ensure after policy rotation; keep last-good marketing sync (48h).
...
begin
  ...
  if not found then
    ...
    -- Policy was rotated (e.g. hard-cap change): rebind/create snapshot for new policy.
    begin
      v_snapshot := public.ensure_meta_organic_boost_exposure_snapshot(
        p_platform_account_id,
        p_user_id,
...
comment on function public.run_meta_organic_boost_planner(uuid, uuid, uuid, uuid, timestamptz) is
  'Materialize organic boost plans; returns structured JSON on failure instead of raising. Rebinds exposure snapshot after policy rotation.';
