-- Beitrag-Push Ampel must keep finished campaigns visible for evaluation.
-- Marketing sync marks campaigns is_current=false when Meta omits them from the
-- account edge (completed/archived/deleted). The Ampel join previously required
-- is_current and then lost status → dashboard looked "active"/empty.
-- Prefer current row; fall back to newest local row by Meta campaign id.
...
begin
  ...
  bound as (
    ...
    left join lateral (
      ...
      order by c.is_current desc, c.updated_at desc nulls last, c.created_at desc nulls last
      limit 1
    ) campaign on true
  ),
  adset_live as (
...
comment on function public.list_meta_organic_boost_campaigns(uuid) is
  'Organic Ampel: keep finished/non-current campaigns for evaluation; prefer is_current then latest local row.';

-- Keep Beitrag-Push campaign rows current after marketing sync soft-expires them.
create or replace function public.retain_meta_organic_boost_campaigns(
  p_platform_account_id uuid,
  p_user_id uuid
)
returns jsonb
...
begin
  if p_platform_account_id is null or p_user_id is null then
    return jsonb_build_object('retained', 0, 'completed', 0);
  end if;

  with boost_ids as (
    select distinct binding.remote_object_id as platform_campaign_id
    from public.meta_organic_boost_links link
  ...
  touched as (
    ...
    set
      ...
      effective_status = case
        when upper(coalesce(c.effective_status, '')) in (
          'COMPLETED', 'CAMPAIGN_COMPLETED', 'ARCHIVED', 'DELETED'
        ) then c.effective_status
        when c.stop_time is not null and c.stop_time <= now()
          and upper(coalesce(c.effective_status, c.status, '')) in ('ACTIVE', '')
          then 'COMPLETED'
        else c.effective_status
      end,
      updated_at = now()
    from boost_ids b
    where c.user_id = p_user_id
      and c.platform_account_id = p_platform_account_id
      and c.platform_campaign_id = b.platform_campaign_id
    returning
      c.id,
      upper(coalesce(c.effective_status, '')) as effective_status
  )
  select
    count(*)::integer,
    count(*) filter (
...
revoke all on function public.retain_meta_organic_boost_campaigns(uuid, uuid)
  from public, anon, authenticated;
grant execute on function public.retain_meta_organic_boost_campaigns(uuid, uuid)
  to service_role;

comment on function public.retain_meta_organic_boost_campaigns(uuid, uuid) is
  'After marketing sync, keep Beitrag-Push campaigns current and mark schedule-ended rows COMPLETED.';
