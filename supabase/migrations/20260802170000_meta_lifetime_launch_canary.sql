-- Native campaign lifetime-budget support for the atomic Meta launch canary.
-- Daily-v2 remains unchanged; v3 is additive and requires fixed runtime,
-- full-budget exposure reservation, immutable approval facts and remote
-- budget/time read-back before the first ACTIVE write.

-- Contract v3 adds native campaign-level lifetime budgets without changing the
-- existing Daily-v2 materializer, approval RPC, or executor semantics.
...
alter table public.meta_launch_canary_approvals
  alter column daily_budget_minor drop not null;

alter table public.meta_launch_canary_approvals
  drop constraint if exists meta_launch_canary_approvals_budget_contract_check;
alter table public.meta_launch_canary_approvals
  add constraint meta_launch_canary_approvals_budget_contract_check check (
    (
      budget_type = 'DAILY'
      ...
      and end_time <= start_time + interval '90 days'
    )
  );

create or replace function public.meta_active_lifetime_budget_exposure_minor(
  p_user_id uuid,
...
set search_path = ''
as $$
  select coalesce(sum(owner.remaining_minor), 0)::bigint
  from (
    select least(
      campaign.lifetime_budget_minor,
      coalesce(campaign.budget_remaining_minor, campaign.lifetime_budget_minor)
    )::bigint as remaining_minor
    from public.campaigns campaign
    where campaign.user_id = p_user_id
      and campaign.platform_account_id = p_platform_account_id
      and campaign.last_seen_sync_id = p_source_marketing_sync_id
      and campaign.is_current
      and upper(coalesce(campaign.effective_status, campaign.status, '')) = 'ACTIVE'
      and coalesce(campaign.daily_budget_minor, 0) = 0
      and coalesce(campaign.lifetime_budget_minor, 0) > 0
      and coalesce(campaign.budget_remaining_minor, campaign.lifetime_budget_minor) > 0
      and (campaign.start_time is null or campaign.start_time <= p_observed_at)
      and (campaign.stop_time is null or campaign.stop_time > p_observed_at)

    union all

    select least(
      ad_set.lifetime_budget_minor,
      coalesce(ad_set.budget_remaining_minor, ad_set.lifetime_budget_minor)
    )::bigint as remaining_minor
    from public.ad_groups ad_set
    join public.campaigns campaign on campaign.id = ad_set.campaign_id
    where ad_set.user_id = p_user_id
      and ad_set.platform_account_id = p_platform_account_id
      and ad_set.last_seen_sync_id = p_source_marketing_sync_id
      and ad_set.is_current
      and upper(coalesce(ad_set.effective_status, ad_set.status, '')) = 'ACTIVE'
      ...
      and coalesce(campaign.daily_budget_minor, 0) = 0
      and coalesce(campaign.lifetime_budget_minor, 0) = 0
      and coalesce(ad_set.daily_budget_minor, 0) = 0
      and coalesce(ad_set.lifetime_budget_minor, 0) > 0
      and coalesce(ad_set.budget_remaining_minor, ad_set.lifetime_budget_minor) > 0
      and (campaign.start_time is null or campaign.start_time <= p_observed_at)
      and (campaign.stop_time is null or campaign.stop_time > p_observed_at)
      and (ad_set.start_time is null or ad_set.start_time <= p_observed_at)
      and (ad_set.end_time is null or ad_set.end_time > p_observed_at)
  ) owner;
$$;

revoke all on function public.meta_active_lifetime_budget_exposure_minor(
  uuid, uuid, uuid, timestamptz
) from public, anon, authenticated;
grant execute on function public.meta_active_lifetime_budget_exposure_minor(
  uuid, uuid, uuid, timestamptz
) to service_role;

create or replace function public.reserve_meta_lifetime_budget_exposure_v3(
  p_user_id uuid,
  p_platform_account_id uuid,
  p_policy_id uuid,
  p_snapshot_id uuid,
...
begin
  if p_plan_id is null
    or p_account_day is null
    or p_campaign_scope_key is null
    or p_budget_owner_key is null
    or p_lifetime_budget_minor is null
    or p_lifetime_budget_minor <= 0
    or p_source not in ('PLAN', 'RECONCILIATION') then
    raise exception 'Invalid Contract-v3 lifetime exposure reservation';
  end if;
  ...
  if not exists (
    ...
    where snapshot.id = p_snapshot_id
      ...
      and snapshot.account_day = p_account_day
      and snapshot.currency = p_currency
  ...
  where plan.id = p_plan_id
    ...
    and plan.policy_id = p_policy_id
    and plan.action_type = 'LAUNCH_CHAIN'
    and plan.target_type = 'CHAIN'
  for share;

  if not found
    or v_plan.planned_payload->>'contract_version' is distinct from '3'
    or v_plan.planned_payload->>'budget_type' is distinct from 'LIFETIME'
    or v_plan.planned_payload->>'budget_owner_type' is distinct from 'CAMPAIGN'
    or v_plan.planned_payload->>'lifetime_budget_minor'
         is distinct from p_lifetime_budget_minor::text
    or v_plan.expected_before->>'exposure_snapshot_id'
         is distinct from p_snapshot_id::text then
    raise exception 'Contract-v3 lifetime exposure is not bound to its immutable plan';
  end if;

  if p_source = 'PLAN' then
    if p_automation_target_id is not null
      or v_plan.status <> 'PENDING'
      or v_plan.not_before <> 'infinity'::timestamptz
      or v_plan.max_attempts <> 1
      or v_plan.planned_payload->>'provisional_campaign_scope_key'
           is distinct from p_campaign_scope_key
      or v_plan.planned_payload->>'provisional_budget_owner_key'
           is distinct from p_budget_owner_key then
      raise exception 'Provisional Contract-v3 lifetime exposure binding is invalid';
    end if;
  elsif p_automation_target_id is null
    or v_plan.status not in ('EXECUTING', 'RECONCILING')
    or not exists (
      select 1
      ...
      where target.id = p_automation_target_id
        ...
        and target.campaign_scope_key = p_campaign_scope_key
        and target.budget_owner_type = 'CAMPAIGN'
        and target.budget_owner_key = p_budget_owner_key
    ) then
    raise exception 'Canonical Contract-v3 lifetime exposure binding is invalid';
  end if;

  perform 1
  from public.daily_budget_exposures exposure
  where exposure.platform_account_id = p_platform_account_id
    and exposure.account_day = p_account_day
  for update;
  ...
  insert into public.daily_budget_exposures (
    ...
    p_budget_owner_key, 'CAMPAIGN', false, p_currency,
    p_lifetime_budget_minor, 10000, p_source, now(), now()
  )
  ...
  left join public.campaign_budget_limits campaign_limit
    on campaign_limit.policy_id = p_policy_id
   and campaign_limit.user_id = p_user_id
   and campaign_limit.platform_account_id = p_platform_account_id
   and campaign_limit.campaign_scope_key = p_campaign_scope_key;
  ...
  where exposure.platform_account_id = p_platform_account_id
    and exposure.account_day = p_account_day
    and exposure.campaign_scope_key = p_campaign_scope_key;
...
revoke all on function public.reserve_meta_lifetime_budget_exposure_v3(
  uuid, uuid, uuid, uuid, uuid, uuid, date, text, text, text, bigint, text
) from public, anon, authenticated;
grant execute on function public.reserve_meta_lifetime_budget_exposure_v3(
  uuid, uuid, uuid, uuid, uuid, uuid, date, text, text, text, bigint, text
) to service_role;

comment on function public.reserve_meta_lifetime_budget_exposure_v3(
  uuid, uuid, uuid, uuid, uuid, uuid, date, text, text, text, bigint, text
) is
  'Reserves one immutable Contract-v3 campaign lifetime budget at exactly 1.00x while preserving policy campaign and account hard caps.';
...
begin
  ...
  select count(*) filter (where remote_status = 'PAUSED')::integer
  into v_paused_types
  from latest_before_barrier;

  if (v_plan.planned_payload->>'contract_version')::integer = 3 then
    select snapshot.snapshot_payload
    into v_campaign_snapshot
    from public.meta_mutation_remote_snapshots snapshot
    join public.mutation_plan_steps snapshot_step on snapshot_step.id = snapshot.step_id
    where snapshot.plan_id = p_plan_id
      and snapshot.object_type = 'CAMPAIGN'
      and snapshot.snapshot_kind in ('READ_AFTER_WRITE', 'AMBIGUITY_PROBE')
      and snapshot_step.step_index < v_step.step_index
    order by snapshot_step.step_index desc, snapshot.observed_at desc, snapshot.created_at desc
    limit 1;

    select snapshot.snapshot_payload
    into v_ad_set_snapshot
    from public.meta_mutation_remote_snapshots snapshot
    join public.mutation_plan_steps snapshot_step on snapshot_step.id = snapshot.step_id
    where snapshot.plan_id = p_plan_id
      and snapshot.object_type = 'AD_SET'
      and snapshot.snapshot_kind in ('READ_AFTER_WRITE', 'AMBIGUITY_PROBE')
      and snapshot_step.step_index < v_step.step_index
    order by snapshot_step.step_index desc, snapshot.observed_at desc, snapshot.created_at desc
    limit 1;
    ...
    begin
      if v_campaign_snapshot is null
        or v_ad_set_snapshot is null
        or coalesce(nullif(v_campaign_snapshot->>'daily_budget', ''), '0')::bigint <> 0
        or (v_campaign_snapshot->>'lifetime_budget')::bigint
             <> (v_plan.planned_payload->>'lifetime_budget_minor')::bigint
        or coalesce(nullif(v_ad_set_snapshot->>'daily_budget', ''), '0')::bigint <> 0
        or coalesce(nullif(v_ad_set_snapshot->>'lifetime_budget', ''), '0')::bigint <> 0
        or (v_ad_set_snapshot->>'start_time')::timestamptz
             <> (v_plan.planned_payload->>'start_time')::timestamptz
        or (v_ad_set_snapshot->>'end_time')::timestamptz
             <> (v_plan.planned_payload->>'end_time')::timestamptz then
        return false;
      end if;
...
-- Contract-v3 launch reconciliation supports campaign lifetime budgets
-- while preserving the exact Daily-v2 reconciliation branch and exposure multiplier.
...
revoke all on function public.materialize_meta_customer_lifetime_launch_plan_v3(
  uuid, uuid, uuid, uuid, uuid, uuid, uuid, text, bigint,
  timestamptz, timestamptz, jsonb, timestamptz
) from public, anon, authenticated;
grant execute on function public.materialize_meta_customer_lifetime_launch_plan_v3(
  uuid, uuid, uuid, uuid, uuid, uuid, uuid, text, bigint,
  timestamptz, timestamptz, jsonb, timestamptz
) to service_role;
...
comment on function public.materialize_meta_customer_lifetime_launch_plan_v3(
  uuid, uuid, uuid, uuid, uuid, uuid, uuid, text, bigint,
  timestamptz, timestamptz, jsonb, timestamptz
) is
  'Prepares an immutable held Contract-v3 launch with campaign lifetime budget and fixed Ad Set runtime; performs no Meta write.';
comment on function public.approve_meta_lifetime_launch_canary_plan_v3(
  uuid, uuid, uuid, text, text, text, text, bigint, timestamptz, timestamptz,
  text, text, text, text, text, text
) is
  'Atomically approves one exact Contract-v3 lifetime launch after fingerprint, exposure, brand, domain, step graph and freeze checks.';
