-- Meta Active Launch Chain v1.
-- Materializes a tenant-safe, immutable launch saga from confirmed policy,
-- domain, objective blueprint, brand profile and brand-asset inputs. Extends
-- reconciliation with full local projections and canonical remote bindings.

create or replace function public.meta_launch_payload_keys_allowed(
  p_object_type text,
  p_payload jsonb
)
returns boolean
...
begin
  ...
  v_allowed := case p_object_type
    when 'CAMPAIGN' then array[
      'adlabels', 'bid_strategy', 'budget_schedule_specs', 'buying_type',
      'campaign_optimization_type', 'daily_budget',
      'is_adset_budget_sharing_enabled', 'is_skadnetwork_attribution',
      'is_using_l3_schedule', 'iterative_split_test_configs',
      'lifetime_budget', 'name', 'objective', 'pacing_type',
      'promoted_object', 'smart_promotion_type', 'source_campaign_id',
      'special_ad_categories', 'special_ad_category_country', 'spend_cap',
      'status', 'topline_id'
    ]::text[]
    when 'AD_SET' then array[
      'adlabels', 'adset_schedule', 'attribution_spec', 'bid_adjustments',
      'bid_amount', 'bid_constraints', 'bid_strategy', 'billing_event',
      'campaign_id', 'campaign_spec', 'daily_budget',
      'daily_min_spend_target', 'daily_spend_cap', 'destination_type',
      'end_time', 'frequency_control_specs', 'instagram_user_id',
      'is_dynamic_creative', 'lifetime_budget', 'lifetime_imps',
      'lifetime_min_spend_target', 'lifetime_spend_cap', 'name',
      'optimization_goal', 'optimization_sub_event', 'pacing_type',
      'promoted_object', 'regional_regulated_categories', 'start_time',
      'status', 'targeting', 'time_based_ad_rotation_id_blocks',
      'time_based_ad_rotation_intervals'
    ]::text[]
    when 'CREATIVE' then array[
      'adlabels', 'applink_treatment', 'asset_feed_spec',
      'authorization_category', 'branded_content_sponsor_page_id',
      'call_to_action_type', 'categorization_criteria',
      'collaborative_ads_lsb_image_bank_id', 'contextual_multi_ads',
      'creative_sourcing_spec', 'degrees_of_freedom_spec',
      'destination_set_id', 'dynamic_ad_voice', 'enable_direct_install',
      'enable_launch_instant_app', 'facebook_branded_content',
      'format_transformation_spec', 'generative_asset_spec', 'image_crops',
      'image_hash', 'image_url', 'instagram_branded_content',
      'instagram_permalink_url', 'instagram_user_id',
      'link_destination_display_url', 'link_url',
      'marketing_message_structured_spec', 'media_sourcing_spec',
      'messenger_sponsored_message', 'name', 'object_id',
      'object_store_url', 'object_store_urls', 'object_story_id',
      'object_story_spec', 'object_type', 'object_url',
      'platform_customizations', 'portrait_customizations',
      'recommender_settings', 'source_instagram_media_id', 'template_url',
      'template_url_spec', 'thumbnail_url', 'url_tags', 'video_id'
    ]::text[]
    when 'AD' then array[
      'ad_schedule_end_time', 'ad_schedule_start_time', 'adlabels',
      'adset_id', 'audience_id', 'authorization_category',
      'conversion_domain', 'creative', 'creative_asset_groups_spec',
      'creative_automation_spec', 'dataset_split_specs', 'date_format',
      'display_sequence', 'engagement_audience', 'include_demolink_hashes',
  ...
  for v_key in
    select allowed_key.value
    from pg_catalog.jsonb_object_keys(p_payload) as allowed_key(value)
  loop
...
begin
  ...
  if coalesce(v_kill_mode, 'ALLOW') <> 'ALLOW' then
    raise exception 'Meta launch is blocked by kill switch';
  end if;
  ...
  insert into public.mutation_plans (
    ...
    'PENDING', 60, false, p_planned_at, 5,
  ...
  insert into public.mutation_plan_steps (
    ...
    v_index, 'validate-ad-active', 'VALIDATE', 'AD', v_previous_step,
    v_request, public.meta_sha256(v_request::text),
    jsonb_build_object('validated', true), 'NONE', 'PENDING'
  );
  ...
  insert into public.mutation_plan_steps (
    ...
    v_index, 'create-ad-active-shadow', 'CREATE', 'AD', v_previous_step,
    v_request, public.meta_sha256(v_request::text),
  ...
  insert into public.mutation_plan_steps (
    ...
    v_index, 'read-ad-active-shadow', 'READ', 'AD', v_previous_step,
    v_request, public.meta_sha256(v_request::text),
  ...
  insert into public.mutation_plan_steps (
    ...
    v_index, 'assert-ad-active', 'UPDATE', 'AD', v_previous_step,
    v_request, public.meta_sha256(v_request::text),
    jsonb_build_object('status', 'ACTIVE'), 'PAUSE', 'PENDING'
  );
...
-- Preserve the fully tested UPDATE_BUDGET / status reconciliation as a base
-- function and route only launch plans through the projection materializer.
alter function public.reconcile_meta_mutation_plan(uuid, uuid, uuid)
  rename to reconcile_meta_mutation_plan_base;
...
begin
  ...
  if coalesce(v_campaign_snapshot.snapshot_payload->>'daily_budget', '') ~ '^[0-9]+$' then
    v_campaign_budget := (v_campaign_snapshot.snapshot_payload->>'daily_budget')::bigint;
  end if;
  if coalesce(v_ad_set_snapshot.snapshot_payload->>'daily_budget', '') ~ '^[0-9]+$' then
    v_ad_set_budget := (v_ad_set_snapshot.snapshot_payload->>'daily_budget')::bigint;
  end if;
  ...
  insert into public.campaigns (
    ...
    v_campaign_budget,
    case
      when coalesce(v_campaign_snapshot.snapshot_payload->>'lifetime_budget', '') ~ '^[0-9]+$'
        then (v_campaign_snapshot.snapshot_payload->>'lifetime_budget')::bigint
      else null
    end,
  ...
  insert into public.ad_groups (
    ...
    v_ad_set_budget,
    case
      when coalesce(v_ad_set_snapshot.snapshot_payload->>'lifetime_budget', '') ~ '^[0-9]+$'
        then (v_ad_set_snapshot.snapshot_payload->>'lifetime_budget')::bigint
      else null
    end,
    null,
    null,
  ...
  perform public.append_meta_mutation_audit_event(
    ...
    jsonb_build_object(
      ...
      'daily_budget_minor', v_expected_budget
    ),
    jsonb_build_object(
      'campaign_snapshot_id', v_campaign_snapshot.id,
      'ad_set_snapshot_id', v_ad_set_snapshot.id,
...
declare
  v_action_type text;
begin
  select mp.action_type into v_action_type
  from public.mutation_executions me
...
revoke all on function public.meta_launch_payload_keys_allowed(text, jsonb)
...
revoke all on function public.reconcile_meta_mutation_plan_base(
  uuid, uuid, uuid
) from public, anon, authenticated;
revoke all on function public.reconcile_meta_mutation_plan(
  uuid, uuid, uuid
) from public, anon, authenticated;
...
grant execute on function public.reconcile_meta_mutation_plan(
  uuid, uuid, uuid
) to service_role;

comment on function public.materialize_meta_launch_chain_plan(
  uuid, uuid, uuid, uuid, uuid, uuid, uuid, uuid, uuid[], uuid,
  text, bigint, jsonb, timestamptz
) is
  'Service-role-only materializer for an immutable PAUSED-shadow Meta Active Launch saga.';
comment on function public.reconcile_meta_mutation_plan(uuid, uuid, uuid) is
  'Executor reconciliation router: launch chains materialize complete local projections; other actions use the tested base reconciler.';
