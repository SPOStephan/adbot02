-- Harden Beitrag-Push finalize: campaign ACTIVE alone is never enough.
-- Require AD + AD_SET bindings and activate-ad / activate-ad-set REMOTE_APPLIED
-- (drop the previous "or step absent" loophole).
...
comment on function public.finalize_meta_organic_boost_already_active_plans(uuid, uuid) is
  'SUCCEEDED only when campaign effective ACTIVE and activate-ad-set + activate-ad are REMOTE_APPLIED with AD/AD_SET bindings present.';
