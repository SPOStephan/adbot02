-- Harden organic planner against PostgreSQL
-- "could not determine polymorphic type because input has type unknown"
-- (bare null / to_jsonb(null) / untyped coalesce). Also keep planner
-- returning structured JSON so Beitrag-Push can continue after soft failures.
...
begin
  ...
  if not found then
    ...
    v_detail := jsonb_build_object(
      'status', v_status,
      'last_error', to_jsonb(
        'Meta-Konto nicht bereit (EUR, ads_management oder Marketing-Sync >48h).'::text
      )
    );
    update public.platform_accounts
    set
...
comment on function public.run_meta_organic_boost_planner(uuid, uuid, uuid, uuid, timestamptz) is
  'Materialize organic boost plans; typed JSON on all paths (no polymorphic unknown nulls).';
