-- Ampel spend/impressions joined only on local campaign UUID. After marketing
-- sync, performance_data often attaches to the current campaigns row while
-- remote_object_bindings.local_campaign_id still points at an older row →
-- spend drops to empty/zero on reload. Aggregate by Meta platform_campaign_id.
...
begin
  ...
  bound as (
    select
      boost.link_id,
      -- Prefer current campaigns row (by Meta id), not a stale local_campaign_id.
...
comment on function public.list_meta_organic_boost_campaigns(uuid) is
  'Organic Ampel: spend/impressions by Meta platform_campaign_id so marketing sync remaps cannot blank metrics.';
