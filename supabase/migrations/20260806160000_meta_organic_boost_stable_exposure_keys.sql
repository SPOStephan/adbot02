-- Organic boost hard-cap: do not stack a new daily exposure reservation on every
-- Abruf/snapshot rotation. Bind provisional budget keys to the content candidate
-- and clear stale boost:* PLAN exposures that are not tied to an active plan.
...
-- One-shot cleanup for already-stacked ghost reservations from earlier attempts.
delete from public.daily_budget_exposures dbe
where dbe.source = 'PLAN'
  and (
...
comment on function public.materialize_meta_organic_boost_plan(
  ...
  'Materialize one organic boost plan. Provisional exposure keys are stable per content candidate so Abruf/snapshot rotation cannot stack hard-cap reservations.';
