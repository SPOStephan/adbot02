-- Meta said 6 PAUSED but force-reactivate reported 0 candidates.
-- Cause: candidate discovery required mutation_plans.action_type = 'LAUNCH_CHAIN'
-- on the linked plan; status refresh only needs meta_organic_boost_links +
-- bindings. Also: campaign status updates could no-op (0 rows) while counting
-- as refreshed.
--
-- Fix: discover via links+bindings (no LAUNCH_CHAIN gate), and add an explicit
-- by-ids RPC the app calls with the Meta-PAUSED platform campaign ids.
...
begin
  ...
  -- Beitrag-Push = link+binding, not necessarily still LAUNCH_CHAIN on plan row.
  if not exists (
    select 1
  ...
  v_idempotency_key := public.meta_sha256(
    ...
    || p_account_day::text || '|organic-boost-reactivate-v2|'
...
-- Explicit path: reactivate these Meta campaign ids (already marked PAUSED locally).
create or replace function public.force_reactivate_meta_organic_boost_by_ids(
  p_platform_account_id uuid,
  p_user_id uuid,
  p_source_marketing_sync_id uuid,
  p_platform_campaign_ids text[],
...
begin
  if p_planned_at < now() - interval '5 minutes'
    ...
    return jsonb_build_object(
      ...
      'created', 0, 'existing', 0, 'blocked', 0,
      'schedule_ended', 0, 'candidates', 0, 'requested', 0,
      'targets_repaired', 0
    );
  end if;

  if p_platform_campaign_ids is null
    or coalesce(cardinality(p_platform_campaign_ids), 0) < 1 then
    return jsonb_build_object(
      'outcome', 'OK',
      'reason', 'no_platform_campaign_ids',
      'created', 0, 'existing', 0, 'blocked', 0,
      'schedule_ended', 0, 'candidates', 0, 'requested', 0,
      'targets_repaired', 0
    );
  ...
  foreach v_platform_campaign_id in array p_platform_campaign_ids
  loop
    if v_platform_campaign_id is null
      or length(trim(v_platform_campaign_id)) < 1 then
      continue;
    end if;
    ...
    where c.user_id = p_user_id
      and c.platform_account_id = p_platform_account_id
      and c.platform_campaign_id = v_platform_campaign_id
      and c.is_current
    for update;
    ...
    if upper(coalesce(v_campaign.status, '')) <> 'PAUSED'
      and upper(coalesce(v_campaign.effective_status, ''))
    ...
    v_scope_key := 'campaign:' || v_platform_campaign_id;
    ...
    insert into public.automation_targets (
      ...
      v_platform_campaign_id, v_scope_key, 'CAMPAIGN', v_scope_key,
      v_campaign.id, null, null, 'MANAGED',
    ...
    if v_target_id is null then
      ...
      where target.platform_account_id = p_platform_account_id
        and target.target_type = 'CAMPAIGN'
        and target.platform_object_id = v_platform_campaign_id;
    end if;
    ...
    v_result := public.queue_meta_organic_boost_reactivate_internal(
      ...
      jsonb_build_object(
        ...
        'resume_reason', 'organic_boost_reactivate_by_ids',
  ...
  return jsonb_build_object(
    'outcome', 'OK',
    'reason', 'organic_boost_reactivate_by_ids',
    'created', v_created,
    'existing', v_existing,
    'blocked', v_blocked,
...
revoke all on function public.force_reactivate_meta_organic_boost_by_ids(
  uuid, uuid, uuid, text[], timestamptz
) from public, anon, authenticated;
grant execute on function public.force_reactivate_meta_organic_boost_by_ids(
  uuid, uuid, uuid, text[], timestamptz
) to service_role;

comment on function public.force_reactivate_meta_organic_boost_by_ids(
  uuid, uuid, uuid, text[], timestamptz
) is
  'Queues ACTIVATE for explicit Meta-PAUSED Beitrag-Push campaign ids; repairs MANAGED automation_targets.';

-- Also loosen the scan-based force reactivate: links+bindings only.
...
declare
  v_ids text[];
begin
  select coalesce(array_agg(distinct binding.remote_object_id), '{}'::text[])
  into v_ids
  from public.meta_organic_boost_links link
  join public.remote_object_bindings binding
    on binding.plan_id = link.plan_id
  ...
  where link.user_id = p_user_id
    and link.platform_account_id = p_platform_account_id
    and binding.remote_object_id is not null
    and (
  ...
  return public.force_reactivate_meta_organic_boost_by_ids(
    p_platform_account_id,
    p_user_id,
    p_source_marketing_sync_id,
    v_ids,
